%% Fourier Transforms - White Noise

% Finds the fourier transform of the mean data
% Takes significant periods based on White Noise
...
% Main Directory
Main_Dir = 'C:\Users\Trevor\Documents\College Stuff\Research\Hunter Research';

% Model Folder - List of Main Folders for the models
Model_Fol = {'catskills_sta_data'}; %,'cmip5_may15_2018'};
...
file_name = {'catskill_compare_stations_june2017_PRCP_combined_Annual_Mean',...
    };
%              'tasmin_day_CCSM4_historical_r1i1p1_Seasonal_Mean_Summer',...
%              'tasmin_day_CCSM4_historical_r2i1p1_Seasonal_Mean_Summer',...
%              'tasmin_day_CCSM4_historical_r6i1p1_Seasonal_Mean_Summer',...
%              'tasmin_day_HadGEM2-CC_historical_r1i1p1_Seasonal_Mean_Summer',...
%              'tasmin_day_HadGEM2-CC_historical_r2i1p1_Seasonal_Mean_Summer',...
%              'tasmin_day_HadGEM2-CC_historical_r3i1p1_Seasonal_Mean_Summer',...
%              'tasmin_day_CanESM2_historical_r1i1p1_Seasonal_Mean_Summer',...
...
for file_idx = 1:length(file_name)
    for fol_idx = 1:length(File_Fol)
        for model_idx = 1:length(Model_Fol)
            ...
            % Getting the proper set of years
            ...
            % Peaks and Max
            locs_90 = find(P1(2:end) > P1_white_90(2:end)); %90 percentile
            locs_90 = locs_90 + 1;
            pks_90 = P1(locs_90);
            
            locs_99 = find(P1(2:end) > P1_white_99(2:end)); %99 percentile
            locs_99 = locs_99 + 1;
            pks_99 = P1(locs_99);
            ...
            if (save_tag == 1)
                ...
                save_folder = strcat(save_folder,filesep,'Fourier_Transform_White_Noise');
                if (exist(save_folder,'dir') == 0)
                    mkdir(save_folder)
                end   
            ...
            if (save_tag == 1)
                ...
                save_PNG_folder = strcat(save_PNG_folder,filesep,'Fourier_Transform_White_Noise');
                if (exist(save_PNG_folder,'dir') == 0)
                    mkdir(save_PNG_folder)
                end
            ...
            plot(f(2:end),P1_white_99(2:end),'k--')
            title(title_str)
            xlabel('Frequency')
            ylabel(strcat('Amplitude -',32,y_string))
            legend('Data','Red Noise - 99%','Red Noise - 90%','White Noise - 90%')
            ...
            if ~isempty(pks_90)
                ...
                title({strcat(legend_name,32,plot_type,32,'Combined Peak Frequencies'),...
            ...
            if ~isempty(idx2)
                ...
                title({strcat(legend_name,32,plot_type,32,'Most Correlated Period'),...
                        'Positive Frequency',strcat('Positive Phase',32,num2str(b))})
                xlabel('Year')
                ylabel(y_string)
            ...
            if ~isempty(pks_90)
                ...
                title(strcat(legend_name,32,plot_type,32,'Periods vs Amplitudes'))
                xlabel('Period - Years')
                ylabel(strcat('Amplitude -',32,y_string))
                xlim([0, ceil(L/5)*5])
                fignum = fignum + 1;
            ...
            clearvars -except gong_flag folder_name P_idx Period_Array Main_Dir Model_Fol File_Fol file_name save_tag file_idx fol_idx model_idx fignum Main_plot_type
        end
    end
end
...
% Save location
if (save_tag == 1)
    save_folder = strcat(Main_Dir,filesep,'Fourier Analysis White Noise');
    if (exist(save_folder,'dir') == 0)
        mkdir(save_folder)
    end
...
title({title_str;'Period vs. Amplitude'})
legend(names_str)
ylabel('Period - yrs')
zlabel('Amplitude')
xlabel('Data/Model')
